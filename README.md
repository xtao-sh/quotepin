...
批注工作台（英文名 Quotepin）是一款本地优先的文档审阅应用。它可以导入 PDF、图片、文本和
Office 文档，在页面坐标或 PDF 文字层上添加批注，把项目组织成分组和子项目，导出审阅
...
- 复制了文件之后，在项目页面直接按 Cmd+V 就能导入。剪贴板里是截图这类没有源文件的内容时也能导入，
  只是那样的文档没有可跟踪的原文件，之后不能「刷新」
- 「从文件夹导入」先只读取文件列表：按类型（PDF、Office、图片、文本）缩小范围，勾掉不要的，再一次导入。
  已经在工作区里的文件会标出来并默认跳过，导入过程逐个报成败，失败的可以单独重试。
  在文档页点「导入同类」会直接列出这份文档所在文件夹里的其他文件
...
仓库自带一份示例工作区：两个分组、四个项目（含一个子项目）、九份文档和十四条批注。
启动应用之后运行：
...
示例文件会被复制到 `~/Documents/批注工作台示例/` 再导入，不会碰你自己的文档。载入之后可以直接试
刷新与批注重新定位、跨文档搜索矛盾、清理重复文档和归档。内容全部虚构，说明见
