...
// 示例课程 and the text layer reports four glyph indices from four unrelated scripts.
...
assert.equal(isUnmappableText("内容概览"), false);
...
assert.equal(isUnmappableText("示例ܽؽ课程"), true);

// A mostly-readable line with one stray symbol is not a broken font.
assert.equal(isUnmappableText("内容概览 · 示例课程 2026 秋 ܽ"), false);
...
{
  const report = analyseTextEncoding("内容概览");
...
assert.equal(unmappableRatio("内容概览"), 0);
assert.equal(unmappableRatio(""), 0);
assert.ok(unmappableRatio("内容概览œୗܽؽ") > 0.4 && unmappableRatio("内容概览œୗܽؽ") < 0.6);
